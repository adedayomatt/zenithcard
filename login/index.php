<?php
require('../resources/master.php');

	 if(isset($_POST['login'])){
		 $cr['email'] = trim(htmlentities($_POST['email']));
		 $cr['password'] = $_POST['password'];
		 if(isset($_POST['stay_logged_in'])){
			 $cr['stay_logged_in'] = 1;
		 }
		 else{
			 $cr['stay_logged_in'] = 0;
		 }
		
		 $login = new Login($cr);
		 $loginReport = $login->verify();
		 if($loginReport == 000){//verificaiton was successfull
			if(isset($_GET['_rdr']) && $_GET['_rdr'] != ''){
				$goto = $_GET['_rdr']; //if user was redirected from another page to login
			}
			else{
				$goto = '../dashboard'; //if not redirected from any page, then go the dashboard
			}
			$tool->redirect_to($goto);
		 }
	}
		if(isset($_COOKIE['psw']) && $_COOKIE['psw'] ==1){//if landed here from redirection because of password change
			$passwordChange = true;
			setcookie('psw','',time()-60,'/'); //clear the cookie
				}
			?>

<!DOCTYPE html>
<html lang="zxx" class="js">
<head>
	<meta charset="utf-8">
	<meta name="author" content="ZenithCard">
	<meta name="theme-color" content="#003300">
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<meta name="description" content="Cryptocurrency Backed Debit Card">
	<!-- Fav Icon  -->
	<link rel="shortcut icon" href="../images/favicon.png">
	<!-- Site Title  -->
	<title>ZenithCard &#8226; Login</title>
	<!-- Vendor Bundle CSS -->
	<link rel="stylesheet" href="../assets/css/vendor.bundle.css">
	
	<!--Boostrap added by Matthew
	<link rel="stylesheet" href="../assets/bootstrap/css/bootstrap.min.css">-->
	
	<!-- Custom styles for this templae -->
	<link rel="stylesheet" href="../assets/css/style.css?ver=1.1">
	<link rel="stylesheet" href="../assets/css/theme-mint.css?ver=1.1">
	
	<script id="mcjs">!function(c,h,i,m,p){m=c.createElement(h),p=c.getElementsByTagName(h)[0],m.async=1,m.src=i,p.parentNode.insertBefore(m,p)}(document,"script","https://chimpstatic.com/mcjs-connected/js/users/ddeabd6520c7f855e7ea0c0b4/96742410b1c65912559289283.js");</script>

	<style>
	.primary-color{
		color: #003300;
	}
		#form-container{
			background-color: rgba(255,255,255,0.7);
			border-radius: 5px;
			margin-top: 30px;
			margin-bottom:20px;
		}
		form{
			padding:20px;
		}
		.login-report-container{
			text-align: center;
			padding: 10px;
		}
		.login-report-container.successfull{
			background-color:#32cd32;
			color: #003300;
		}
		.login-report-container.failed{
			color: red;
			background-color: rgba(200,0,0,0.1);
		}
		
	</style>
</head>
<body class="theme-dark io-azure" data-spy="scroll" data-target="#mainnav" data-offset="80">
	<!-- Header --> 
	<header class="site-header is-sticky">
	
	<?php
	$ref = 'login page';
	require('../resources/global/header.php');
	?>
	
		<!-- Banner/Slider -->
		<div id="header" class="banner banner-full d-flex align-items-center">
			<div class="container">
				<div class="banner-content">
					<div class="row align-items-center mobile-center">
					
					<?php
					if($status == 0){ // If user is not logged in
					?>
						<div class="col-lg-6 col-md-12 order-lg-first">
							<div class="header-txt">
								<h1 class="animated" data-animate="fadeInUp" data-delay="1.55">ZenithCard <br class="d-none d-xl-block"> Crypto Based Card <br class="d-none d-xl-block"> and Application</h1>
								<p class="lead animated" data-animate="fadeInUp" data-delay="1.65">Debit card usuable at payment terminals<br class="d-none d-xl-block"> around the world including ATM and Online </p>
								<ul class="btns animated" data-animate="fadeInUp" data-delay="1.75">
									<!--<li><a href="https://goo.gl/forms/a0CZaF5k8MSNryQk2" class="btn">Register to Contribute</a></li>
									<li><a href="#tokenSale" class="btn btn-alt">TOKEN DISTRIBUTION</a></li>-->
								</ul>
							</div>
						</div><!-- .col  -->
						
						<div class="col-lg-6 col-md-12 order-first res-m-bttm-lg" style="padding-right:0px !important;padding-left:0px !important;">
							<div class="animated" data-animate="fadeInRight" data-delay="1.25">
								<div id="form-container" class="primary-color">
									<div style="background-color: #003300; color:#fff">
										<h3 class="text-center" style="padding: 10px 0px; ">Login</h3>
										<p class="text-right" style="padding-right:10px">Don't have ZenithCard account yet?  <a href="../register">Register Now</a>
									</div>
									<hr class="primary-color" style="margin:0px; padding: 0px">
										<?php
										if(isset($loginReport)){
											switch($loginReport){
												case 000:
												?>
												<div class="login-report-container successfull">
													<p>Login Successful</p>
													<a href="../dashboard" class="btn">GO TO DASHBOARD</a>
												</div>
												<?php
												break;
												case 100:
												?>
												<div class="login-report-container successfull">
													<h3 class="primary-color">Account not Verified Yet!</h3>
													<p>The account you are trying to log in is not verified yet, open your mail <strong>(<?php echo $_POST['email'] ?>)</strong> and check for the verification mail we sent you</p><br/>
													<a href="../account/verify" class="btn">Verify Now</a>
												</div>
												<?php
												break;
												case 101:
												?>
												<div class="login-report-container failed">
													<p>Incorrect Password, check and try again. If you have forgotten your password, you can reset it</p>
													<a href="../account/resetpswd" class="btn">Reset Password</a>
												</div>
												<?php
												break;
												case 102:
												?>
												<div class="login-report-container failed">
													<p>Incorrect email address, you can create new account with <strong><?php echo $_POST['email'] ?></strong></p>
													<a href="../register/?m=<?php echo $_POST['email'] ?>" class="btn">Register <?php echo $_POST['email'] ?></a>
												</div>
												<?php
												break;
											}
										}
										if(isset($passwordChange)){//if landed here from redirection because of password change
											?>
											<div class="login-report-container successfull">
												<p class="text-center">Password changed successfully, you will have to login again</p>
											</div>
										<?php
										}
										?>
										
									<form action="<?php $_PHP_SELF ?>" method="POST">
										<div class="form-group">
											<label for="email" class="text-left">Email Address</label>
											<input type="email" name="email" class="form-control" placeholder="you@example.com" value="<?php echo (isset($_POST['email']) ? $_POST['email'] : "") ?>" required>
										</div>
										
										<div class="form-group">
											<label for="password" class="text-left">Password</label>
											<input type="password" name="password" class="form-control" placeholder="Your password" required>
										</div>
										
										
										<div class="">
											<label>
												<input name="stay_logged_in" type="checkbox" value="1" <?php echo (isset($_POST['stay_logged_in']) ? 'checked' : "") ?>> Keep me logged in
											</label>
										</div>
										
										<input type="submit" name="login" value="Login" class="btn">
										<p class="text-right">Forgot password? <a href="../recoverPassword">Recover password</a>
									</form>
							
								</div>
							</div>
						</div><!-- .col  -->
						
						<?php
					}
					else{
						?>
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="text-center">
								<h3>hey <?php echo $user->firstName ?>, don't worry, You are already Logged In</h3>
								<a href="../dashboard" class="btn">Go to Dashboard</a><br/><br/>
								<a href="../logout" class="btn btn-danger" style="background-color: red !important">Logout</a>
							</div>
						</div>
						
						<?php
					}
					?>
					</div><!-- .row  -->
				</div><!-- .banner-content  -->
			</div><!-- .container  -->
		</div>
		<!-- End Banner/Slider -->
		
	</header>
	<!-- End Header -->
	
	<?php
	require('../resources/global/footer.php');
	?>
	
		<!-- Preloader !remove please if you do not want -->
	<div id="preloader">
		<div id="loader"></div>
		<div class="loader-section loader-top"></div>
   		<div class="loader-section loader-bottom"></div>
	</div>
	<!-- Preloader End -->

	<!-- JavaScript (include all script here) -->
	<script src="../assets/js/jquery.bundle.js"></script>
	<script src="../assets/js/script.js"></script>
	
	<!--Start of Tawk.to Script-->
		<script type="text/javascript">
		var Tawk_API=Tawk_API||{}, Tawk_LoadStart=new Date();
		(function(){
		var s1=document.createElement("script"),s0=document.getElementsByTagName("script")[0];
		s1.async=true;
		s1.src='https://embed.tawk.to/5aaa8d354b401e45400dc354/default';
		s1.charset='UTF-8';
		s1.setAttribute('crossorigin','*');
		s0.parentNode.insertBefore(s1,s0);
		})();
		</script>
<!--End of Tawk.to Script-->


	</body>
</html>